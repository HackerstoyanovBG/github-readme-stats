...
const calculateRank = ({
  ...
  const level = (() => {
    if (normalizedScore < RANK_S_VALUE) return "SSSAA++";
    if (normalizedScore < RANK_DOUBLE_A_VALUE) return "SSS+++";
    if (normalizedScore < RANK_A2_VALUE) return "SSS++";
    if (normalizedScore < RANK_A3_VALUE) return "S++++";
    return "S++++";
  })();
